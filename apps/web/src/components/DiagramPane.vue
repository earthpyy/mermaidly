<template>
  <section class="flex min-h-0 min-w-0 flex-col" :style="{ background: 'var(--bg)' }" aria-label="Rendered diagram">
    <!-- Toolbar -->
    <div
    ...
    >
      <div class="flex items-center gap-1.5">
        ...
        <button
          ...
          :style="editorCollapsed ? { color: 'var(--accent)', background: 'var(--accent-soft)' } : {}"
    ...
    >
      ...
      <div v-if="status === 'idle' && !hasOutput" class="max-w-[360px] p-6 text-center">
        <div
          ...
          :style="{ background: 'var(--bg-elev)', borderColor: 'var(--border)', color: 'var(--fg-muted)' }"
        >
        ...
        <h3 class="mb-1.5 text-[15px] font-medium" :style="{ color: 'var(--fg)' }">No diagram yet</h3>
      ...
      <!-- Error state -->
      <div v-else-if="status === 'error'" class="grid place-items-center" style="position: absolute; inset: 0;">
        <div class="max-w-[360px] p-6 text-center">
          ...
          <h3 class="mb-1.5 text-[15px] font-medium" :style="{ color: 'var(--fg)' }">Couldn't render</h3>
          <p class="text-[13px] leading-relaxed" :style="{ color: 'var(--fg-muted)' }">
            There's a syntax issue in your Mermaid code.
          </p>
          <pre
            class="mt-3 max-h-[140px] overflow-auto rounded-[var(--radius-sm)] border p-[10px_12px] text-left text-[11.5px] break-words whitespace-pre-wrap"
            :style="{
...
const { zoom, zoomIn, zoomOut, fitToScreen, applyTransform, onWheel, onMouseDown, onMouseMove, onMouseUp } = useZoom()

const hasOutput = computed(() => props.svgOutput.length > 0)
const zoomPercent = computed(() => Math.round(zoom.value * 100) + '%')

watch(() => props.svgOutput, async (val) => {
  if (val) {
    await nextTick()
    fitToScreen()
